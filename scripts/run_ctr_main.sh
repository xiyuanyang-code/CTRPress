...
# Usage: bash run_ctr_main.sh <MODEL> <PRESS_METHOD>
# Example: bash run_ctr_main.sh qwen_3_1.7b ctr
# CTR-Press ablation: ctr_refine | ctr_semantic | ctr
...
if [ -z "$MODEL" ] || [ -z "$PRESS_METHOD" ]; then
    echo "Usage: bash run_ctr_main.sh <MODEL> <PRESS_METHOD>"
    echo "  MODEL: pythia | qwen_3_1.7b"
    echo "  PRESS_METHOD: ctr_refine | ctr_semantic | ctr"
    exit 1
fi
